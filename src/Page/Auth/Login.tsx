...
const Login: React.FC = () => {
  ...
  const handleLocalLogin = async () => {
    ...
    try {
      ...
      if (user) {
        // 1. Salva o objeto 'user' inteiro como uma string JSON na chave 'user'
        localStorage.setItem('user', JSON.stringify(user));
      }

      // 2. Salva o token na chave correta que a outra página espera ('firebaseToken')
      localStorage.setItem('firebaseToken', token);

      // Você pode remover os outros setItem se eles não forem usados em outras partes do app
      // ou mantê-los por compatibilidade, se necessário.
      navigate("/Home");
    } catch (error: any) {
      // Tratamento de erros...
    }
