...
import { FaUserEdit, FaCheckCircle, FaExclamationTriangle } from "react-icons/fa"; // Importa ícones de notificação
import { useAuth } from "../../Hook/AuthContext";
import { TbPlugConnected } from "react-icons/tb";
import { useSearchParams, useNavigate, useLocation } from 'react-router-dom'; // Importa hooks do router
...
const Perfil = () => {
  ...
  // Estados para a UI e notificações
    ...
    if (user?._id) {
      try {
        ...
        if (perfilRes.data) {
          ...
          setDadosPessoais({
            ...dadosPessoais, ...perfilData.dadosPessoais,
            dataNascimento: dataNascimentoFormatada,
            cpf: formatCpf(perfilData.dadosPessoais?.cpf || ""),
            cnpj: formatCnpj(perfilData.dadosPessoais?.cnpj || "")
          });
          setDadosOrganizacao(perfilData.dadosOrganizacao || dadosOrganizacao);
        }
  ...
  }, [user?._id, updateUser, dadosOrganizacao, dadosPessoais]);
  ...
  useEffect(() => {
    ...
    if (status === 'success') {
      ...
      fetchProfileAndUser(); // Força a recarga dos dados para refletir a nova conexão
      navigate('/perfil', { replace: true }); // Limpa a URL
  ...
  // FUNÇÃO CENTRALIZADA PARA BUSCAR DADOS ATUALIZADOS DO USUÁRIO E PERFIL
  ...
  }, [user, getImagemPerfilUrl, fetchProfileAndUser]);
  ...
  const handleDisconnectMercadoPago = async () => {
    // Pede uma confirmação clara para o usuário
    const isConfirmed = window.confirm(
      "Você tem certeza que deseja desvincular sua conta do Mercado Pago?\n\n" +
      "AVISO: Você não poderá criar novos eventos e as vendas de eventos futuros poderão ser pausadas até que uma nova conta seja conectada."
    );

    if (isConfirmed && user) {
      try {
        // Chama a nova rota da API que vamos criar no backend
        ...
        // Força a recarga dos dados do usuário para que a UI reflita a mudança
        fetchProfileAndUser();

      } catch (error: any) {
        // Mostra a mensagem de erro vinda do backend (ex: "Você tem eventos ativos")
  ...
  return (
    <div className="perfil-container">
      ...
      {/* EXIBE A NOTIFICAÇÃO NO TOPO DA PÁGINA */}
      {notification && (
      ...
      <div className="perfil-card full-width">
        ...
        <div className="perfil-tab-content">
          {/* LÓGICA CONDICIONAL BASEADA NO 'user' DO AUTHCONTEXT */}
              ...
              <div className="perfil-form-grid">
                ...
                <div className="perfil-form-group">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setDadosPessoais({ ...dadosPessoais, [tipoPessoa]: formattedValue });
                    }}
                ...
                {tipoPessoa === "cpf" && (
                  <div className="perfil-form-group">
                    ...
                    <input type="date" className="perfil-input" value={dadosPessoais.dataNascimento} onChange={(e) => setDadosPessoais({ ...dadosPessoais, dataNascimento: e.target.value })} disabled={!editandoDadosAdicionais} />
                  </div>
                )}
                {tipoPessoa === "cnpj" && (
                  <>
